...
def _delta_2opt(t: Tour, i: int, k: int, d: List[List[float]]) -> float:
    # reverse segment (i+1..k); compute delta by swapping 2 edges
    n = len(t)
    a, b = t[i], t[i + 1]
    c = t[k]
...
def simulated_annealing_tsp(dist: List[List[float]], p: SAParams) -> SAResult:
    ...
    tour = nearest_neighbor_tour(dist)  # good default start
    ...
    while T > p.Tmin:
        if p.max_seconds and (time.time() - t_start) >= p.max_seconds:
            break
